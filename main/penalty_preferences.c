...
penalty_language_t penalty_preferences_load_language(void) {
    if (!s_nvs_ready) return PENALTY_LANGUAGE_EN;
    nvs_handle_t handle = 0;
    if (nvs_open("penalty", NVS_READONLY, &handle) != ESP_OK) return PENALTY_LANGUAGE_EN;
    uint8_t value = PENALTY_LANGUAGE_EN;
    esp_err_t err = nvs_get_u8(handle, "language", &value);
    nvs_close(handle);
    return err == ESP_OK ? penalty_preferences_decode_language(value) : PENALTY_LANGUAGE_EN;
}
