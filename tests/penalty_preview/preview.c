...
static void check_meter(const penalty_model_t *m) {
    ...
    for (unsigned x = 0; x < 192; ++x) {
        ...
        assert(s_pixels[210 * 240 + 24 + x] == rgb565(color));
    }
}
...
static void check_target_grid(const penalty_model_t *m) {
    ...
    for (unsigned target = 0; target < PENALTY_TARGET_COUNT; ++target) {
        ...
        assert(s_pixels[(center_y - 10) * 240 + center_x - 21] == rgb565(expected));
    }
}
...
static void check_foreground(const penalty_model_t *m, uint64_t now) {
    ...
    const uint8_t *data = (s.blue_striker ? penalty_art_blue_striker[s.striker.frame]
                                           : penalty_art_striker[s.striker.frame]).data;
    for (unsigned y = 0; y < 88; ++y) {
        for (unsigned x = 0; x < 88; ++x) {
            unsigned i = y * 88 + x;
            if (data[88 * 88 * 2 + i] != 255) continue;
            unsigned expected = data[2*i] | data[2*i + 1] << 8;
            assert(s_pixels[(s.striker.y + y) * 240 + s.striker.x + x] == expected);
        }
    }
}
...
static void render(penalty_model_t *m, uint64_t now, const char *dir, const char *name) {
    ...
    uint16_t header_before[18 * 240];
    memcpy(header_before, s_pixels, sizeof(header_before));
    s_flushed_pixels = 0;
    assert(lv_display_get_rotation(s_display) == LV_DISPLAY_ROTATION_0);
    ...
    if (m->state == PENALTY_FLIGHT && previous == PENALTY_FLIGHT) {
        assert(s_flushed_pixels < 240 * 165); /* No full pitch redraw for one moving frame. */
    ...
    if (memcmp(s_partial, s_pixels, bytes) != 0) {
        for (size_t i = 0; i < bytes / 2; ++i) {
            ...
            fprintf(stderr, "Dirty mismatch state=%d now=%llu at=(%zu,%zu): %04x != %04x\n",
                    m->state, (unsigned long long)now, i % 240, i / 240, s_partial[i], s_pixels[i]);
            break;
        }
        abort();
    }
...
int main(int argc, char **argv) {
    assert(argc == 2);
    assert(penalty_art_background.data_size == 240 * 240 * 2);
    assert(penalty_art_striker[4].header.cf == LV_COLOR_FORMAT_RGB565A8);
    assert(penalty_art_blue_striker[8].header.cf == LV_COLOR_FORMAT_RGB565A8);
    assert(penalty_art_keeper[6].header.cf == LV_COLOR_FORMAT_RGB565A8);
    lv_init();
    ...
    for (unsigned cycle = 0; cycle < 50; ++cycle) {
        ...
        assert(has_visible_label_text(lv_screen_active(), "按 OK 开始"));
        for (int y = 0; y < 320; ++y) {
            for (int x = 0; x < 240; ++x) {
                if (x >= 42 && x < 198 && y >= 265 && y < 295) continue;
        ...
        assert(has_label_text(lv_screen_active(), "[射门挑战]\n守门挑战\n设置\n说明"));
        if (cycle == 0) {
            m.language = PENALTY_LANGUAGE_EN;
            render(&m, 1, argv[1], "02-title-en");
            assert(has_label_text(lv_screen_active(),
                                  "[SHOOTING CHALLENGE]\nKEEPER CHALLENGE\nSETTINGS\nHELP"));
            m.language = PENALTY_LANGUAGE_ZH_CN;
            render(&m, 1, argv[1], NULL);
        }
        ...
        if (cycle == 0) {
            ...
            for (unsigned d = 0; d < 3; ++d) {
                ...
                m.difficulty = d; m.selection = 2; m.state = PENALTY_TITLE;
                char name[48];
            ...
            m.state = PENALTY_COVER;
            render(&m, t, argv[1], "29-cover-zh");
            assert(has_visible_label_text(lv_screen_active(), "按 OK 开始"));

            m.language = PENALTY_LANGUAGE_EN;
            render(&m, t, argv[1], "30-cover-en");
            assert(has_visible_label_text(lv_screen_active(), "PRESS OK"));
            m.mode = PENALTY_MODE_KEEPER;
            m.completed = m.saves = m.goals = m.perfect = 0;
            m.current = (penalty_shot_t){
                .player_target = PENALTY_TOP_CENTER,
                .opponent_target = PENALTY_TOP_CENTER,
                .green_low = 61, .green_high = 80, .perfect_low = 72,
            };
            m.state = PENALTY_AIM; m.since_ms = t;
            for (unsigned target = 0; target < PENALTY_TARGET_COUNT; ++target) {
                char capture[48];
                m.current.opponent_target = (penalty_direction_t)target;
                snprintf(capture, sizeof(capture), "40-keeper-cue-%u", target);
                render(&m, t + 1, argv[1], capture);
            }
            m.current.opponent_target = PENALTY_TOP_RIGHT;
            m.current.player_target = PENALTY_BOTTOM_RIGHT;
            m.state = PENALTY_CHARGE; m.since_ms = t;
            m.current.timing_value = m.current.perfect_low;
            render(&m, t + 200, argv[1], "41-keeper-timing");
            const char *keeper_names[] = {
                "42-keeper-catch", "43-keeper-parry", "44-keeper-wrong-way",
                "45-keeper-read-miss", "46-keeper-early", "47-keeper-late",
            };
            for (unsigned i = 0; i < sizeof(keeper_names) / sizeof(keeper_names[0]); ++i) {
                m.current.outcome = (penalty_outcome_t)(PENALTY_KEEPER_CATCH + i);
                m.state = PENALTY_FLIGHT; m.since_ms = t;
                render(&m, t + 600, argv[1], keeper_names[i]);
            }
            m.state = PENALTY_SUMMARY; m.completed = 5; m.saves = 3; m.perfect = 2;
            for (unsigned i = 0; i < PENALTY_SHOTS; ++i)
                m.shots[i].outcome = i < 3 ? PENALTY_KEEPER_CATCH : PENALTY_KEEPER_WRONG_WAY;
            render(&m, t, argv[1], "48-keeper-summary");
            m.language = PENALTY_LANGUAGE_ZH_CN;
            render(&m, t, argv[1], "49-keeper-summary-zh");
        }
