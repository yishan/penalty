...
`bsp_lvgl_set_landscape(true)` 切换到 320 × 240 逻辑显示，画面向左转四分之一圈，设备顺时针横握；传入 `false` 恢复竖屏。调用时必须持有 LVGL 锁。该接口使用 esp_lvgl_port 的硬件坐标轴／镜像路径，保持原有 4,800 像素 DMA 缓冲，圆角遮罩随当前逻辑尺寸变化。应用恢复方向及删除页面前必须停止事件生产者。接口成功不代表实物方向／裁剪已验收。
