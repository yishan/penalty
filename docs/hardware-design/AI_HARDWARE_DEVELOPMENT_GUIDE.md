...
`bsp_lvgl_set_landscape(true)` selects a 320 × 240 logical display whose image turns left by a quarter-turn; hold the device clockwise. Pass `false` to restore portrait. Call it only with the LVGL lock held. It uses the esp_lvgl_port hardware axis/mirror path and retains the same 4,800-pixel DMA buffer; the corner mask follows the active logical dimensions. Applications must stop producers before restoring orientation and deleting their screens. A successful API return is not physical orientation/clipping acceptance.
