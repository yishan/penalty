...
This Penalty development branch now boots directly to a portrait game cover, not the hardware-demo menu. See [Penalty](../penalty/README.md) for its current flow and firmware. The baseline descriptions and demo references below describe the upstream foundation; their source remains available but demo pages are not linked into this game build.
