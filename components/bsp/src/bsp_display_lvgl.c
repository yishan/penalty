...
esp_err_t bsp_lvgl_set_landscape(bool landscape) {
    ...
    lv_display_rotation_t rotation = landscape ? LV_DISPLAY_ROTATION_90 : LV_DISPLAY_ROTATION_0;
    if (lv_display_get_rotation(s_disp) == rotation) return ESP_OK;
    lv_display_rotation_t previous = lv_display_get_rotation(s_disp);
    lv_display_set_rotation(s_disp, rotation);
    // esp_lvgl_port 2.9.0 updates MADCTL on the size event but ignores driver
    // return codes. Repeat the same mapping here to surface a transport error.
    esp_err_t err = esp_lcd_panel_swap_xy(bsp_display_panel(), landscape);
    if (err == ESP_OK) err = esp_lcd_panel_mirror(bsp_display_panel(), false, landscape);
    if (err != ESP_OK) lv_display_set_rotation(s_disp, previous);
    return err;
}
